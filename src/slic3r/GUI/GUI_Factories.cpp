...
{
    return category.empty() || (filaments_cnt == 1 && (category == "Extruders" || category == "Wipe options")) ||
...
static SettingsFactory::Bundle FREQ_SETTINGS_BUNDLE_FFF = {
    ...
    {L("Support"),
     {"enable_support", "support_type", "support_threshold_angle", "support_threshold_overlap", "support_base_pattern",
      ...
      "support_expansion", "enable_dynapin_support_optimization", "enable_dynapin_placement_optimization", "dynapin_selected_pins", "dynapin_debug_stage"}},
    // BBS
...
std::map<std::string, std::vector<SimpleSettingData>> SettingsFactory::OBJECT_CATEGORY_SETTINGS =
    {{L("Quality"),
      {{"layer_height", "", 1},
       ...
       {"seam_position", "", 2},
       {"slice_closing_radius", "", 3},
       {"resolution", "", 4},
       {"xy_hole_compensation", "", 5},
       {"xy_contour_compensation", "", 6},
       {"elefant_foot_compensation", "", 7},
       {"make_overhang_printable_angle", "", 8},
       {"make_overhang_printable_hole_size", "", 9},
       {"wall_sequence", "", 10},
       {"precise_z_height", "", 10}
     ...
     {L("Support"),
      {{"brim_type", "", 1},
       {"brim_width", "", 2},
       {"brim_object_gap", "", 3},
       {"brim_use_efc_outline", "", 4},
       {"enable_support", "", 5},
       {"support_type", "", 6},
       {"support_threshold_angle", "", 7},
       {"support_threshold_overlap", "", 8},
       {"support_on_build_plate_only", "", 9},
       {"support_filament", "", 10},
       {"support_interface_filament", "", 11},
       {"support_expansion", "", 12},
       {"support_style", "", 13},
       {"tree_support_brim_width", "", 14},
       {"tree_support_branch_angle", "", 15},
       {"tree_support_branch_angle_organic", "", 16},
       {"tree_support_wall_count", "", 17},
       {"tree_support_branch_diameter_angle", "", 18}, // tree support
       {"support_bottom_z_distance", "", 19},
       {"support_top_z_distance", "", 20},
       {"support_base_pattern", "", 21},
       {"support_base_pattern_spacing", "", 22},
       {"support_interface_top_layers", "", 23},
       {"support_interface_bottom_layers", "", 24},
       {"support_interface_spacing", "", 25},
       {"support_bottom_interface_spacing", "", 26},
       {"support_object_xy_distance", "", 27},
       {"bridge_no_support", "", 28},
       {"max_bridge_length", "", 29},
       {"support_critical_regions_only", "", 30},
       {"support_remove_small_overhang", "", 31},
...
std::vector<SimpleSettingData> SettingsFactory::get_visible_options(const std::string& category, const bool is_part)
{
    /*t_config_option_keys options = {
        ...
        //Infill
        "sparse_infill_density", "sparse_infill_pattern", "top_surface_pattern", "bottom_surface_pattern", "infill_combination",
    "infill_direction", "infill_wall_overlap",
        //speed
    ...
    t_config_option_keys object_options = {
        //Quality
        "layer_height", "initial_layer_print_height", "adaptive_layer_height", "seam_position", "xy_hole_compensation",
    "xy_contour_compensation", "elefant_foot_compensation", "support_line_width",
        //Support
        "enable_support", "support_type", "support_threshold_angle", "support_threshold_overlap", "support_on_build_plate_only",
    "support_critical_regions_only", "enforce_support_layers",
        //tree support
        "tree_support_wall_count",
        //support
        "support_top_z_distance", "support_base_pattern", "support_base_pattern_spacing", "support_interface_top_layers",
    "support_interface_bottom_layers", "support_interface_spacing", "support_bottom_interface_spacing", "support_object_xy_distance",
    ...
    auto sort_func = [](SimpleSettingData& setting1, SimpleSettingData& setting2) { return (setting1.priority < setting2.priority); };
...
SettingsFactory::Bundle SettingsFactory::get_bundle(const DynamicPrintConfig* config,
                                                    bool                      is_object_settings,
                                                    bool                      is_layer_settings /* = false*/)
{
...
static Plater* plater() { return wxGetApp().plater(); }

static ObjectList* obj_list() { return wxGetApp().obj_list(); }

static ObjectDataViewModel* list_model() { return wxGetApp().obj_list()->GetModel(); }

static const Selection& get_selection() { return plater()->get_current_canvas3D(true)->get_selection(); }
...
static wxMenu* create_settings_popupmenu(wxMenu* parent_menu, const bool is_object_settings, wxDataViewItem item /*, ModelConfig& config*/)
{
    ...
    auto get_selected_options_for_category = [categories, item](const wxString& category_name) {
        ...
        if (!category_options.empty() && wxGetSelectedChoices(selections, _L("Select settings"), category_name, names) != -1) {
    ...
    for (auto cat : categories) {
        append_menu_item(
            menu, wxID_ANY, _(cat.first), "",
            [menu, item, get_selected_options_for_category](wxCommandEvent& event) {
                std::vector<std::pair<std::string, bool>> category_options = get_selected_options_for_category(
                    menu->GetLabel(event.GetId()));
                obj_list()->add_category_to_settings_from_selection(category_options, item);
            },
            SettingsFactory::get_category_bitmap(cat.first), parent_menu, []() { return true; }, plater());
    }
...
static void create_freq_settings_popupmenu(wxMenu* menu, const bool is_object_settings, wxDataViewItem item)
{
    ...
    for (auto& category : bundle) {
        ...
        append_menu_item(
            menu, wxID_ANY, _(category.first), "",
            ...
            SettingsFactory::get_category_bitmap(category.first), menu, []() { return true; }, plater());
    }
}
...
void MenuFactory::append_menu_item_set_visible(wxMenu* menu)
{
    ...
    append_menu_item(
        menu, wxID_ANY, has_one_shown ? _L("Hide") : _L("Show"), "",
        [has_one_shown](wxCommandEvent&) { plater()->set_selected_visible(!has_one_shown); }, "", nullptr, []() { return true; }, m_parent);
}
...
#ifdef __WINDOWS__
    append_menu_item(
        menu, wxID_ANY, _L("Delete") + "\t" + _L("Del"), _L("Delete the selected object"),
        [](wxCommandEvent&) { plater()->remove_selected(); }, "menu_delete", nullptr, []() { return plater()->can_delete(); }, m_parent);
#else
    append_menu_item(
        menu, wxID_ANY, _L("Delete") + "\t" + _L("Backspace"), _L("Delete the selected object"),
        [](wxCommandEvent&) { plater()->remove_selected(); }, "", nullptr, []() { return plater()->can_delete(); }, m_parent);
#endif
}
...
wxMenu* MenuFactory::append_submenu_add_handy_model(wxMenu* menu, ModelVolumeType type)
{
    ...
    for (auto& item : {L("Orca Cube"), L("Orca Tolerance Test"), L("3DBenchy"), L("Cali Cat"), L("Autodesk FDM Test"), L("Voron Cube"),
                       L("Stanford Bunny"), L("Orca String Hell")}) {
        append_menu_item(
...
static void append_menu_itemm_add_(
    const wxString& name, GLGizmosManager::EType gizmo_type, wxMenu* menu, ModelVolumeType type, bool is_submenu_item)
{
...
void MenuFactory::append_menu_items_add_volume(wxMenu* menu)
{
    ...
    for (size_t type = 0; type < ADD_VOLUME_MENU_ITEMS.size(); type++) {
        ...
        append_submenu(
            menu, sub_menu, wxID_ANY, _(item.first), "", item.second, []() { return obj_list()->is_instance_or_object_selected(); },
            m_parent);
    }
...
wxMenuItem* MenuFactory::append_menu_item_layers_editing(wxMenu* menu)
{
    return append_menu_item(
        menu, wxID_ANY, _L("Height range Modifier"), "",
        [](wxCommandEvent&) {
            obj_list()->layers_editing();
            wxGetApp().params_panel()->switch_to_object();
        },
        "height_range_modifier", menu, []() { return obj_list()->is_instance_or_object_selected(); }, m_parent);
}
...
wxMenuItem* MenuFactory::append_menu_item_settings(wxMenu* menu_)
{
    ...
    if ((selection.is_multiple_full_instance() && !selection.is_single_full_object()) || selection.is_multiple_volume() ||
        selection.is_mixed()) // more than one volume(part) is selected on the scene
        return nullptr;
    ...
    const wxDataViewItem sel_item = // when all instances in object are selected
        object_list->GetSelectedItemsCount() > 1 && selection.is_single_full_object() ? obj_model->GetItemById(selection.get_object_idx()) :
...
{
    return append_menu_item(
        menu, wxID_ANY, _L("Change type"), "", [](wxCommandEvent&) { obj_list()->change_part_type(); }, "", menu,
        []() {
...
wxMenuItem* MenuFactory::append_menu_item_instance_to_object(wxMenu* menu)
{
    wxMenuItem* menu_item = append_menu_item(
        menu, wxID_ANY, _L("Set as an individual object"), "", [](wxCommandEvent&) { obj_list()->split_instances(); }, "", menu);
...
void MenuFactory::append_menu_item_fill_bed(wxMenu* menu)
{
    append_menu_item(
        ...
        [](wxCommandEvent&) { plater()->fill_bed_with_copies(); }, "", nullptr, []() { return plater()->can_increase_instances(); },
        m_parent);
}
...
{
    // BBS: to be checked
    wxMenuItem* menu_item_printable =
        append_menu_check_item(menu, wxID_ANY, _L("Printable"), "", [](wxCommandEvent&) { obj_list()->toggle_printable_state(); }, menu);
...
{
    append_menu_item(menu, wxID_ANY, _L("Rename"), "", [](wxCommandEvent&) { obj_list()->rename_item(); }, "", menu);
...
wxMenuItem* MenuFactory::append_menu_item_fix_through_netfabb(wxMenu* menu)
{
    ...
    wxMenuItem* menu_item = append_menu_item(
        menu, wxID_ANY, _L("Fix model"), "", [](wxCommandEvent&) { obj_list()->fix_through_netfabb(); }, "", menu,
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Export as one STL") + dots, "", [](wxCommandEvent&) { plater()->export_stl(false, true); }, "", nullptr,
        [is_mulity_menu]() {
            const Selection& selection = plater()->canvas3D()->get_selection();
            if (is_mulity_menu)
    ...
    if (!is_mulity_menu)
        return;
    append_menu_item(
        menu, wxID_ANY, _L("Export as STLs") + dots, "", [](wxCommandEvent&) { plater()->export_stl(false, true, true); }, "", nullptr,
        []() {
            const Selection& selection = plater()->canvas3D()->get_selection();
            return selection.is_multiple_full_instance() || selection.is_multiple_full_object();
        },
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Export as one DRC") + dots, "", [](wxCommandEvent&) { plater()->export_stl(false, true, false, FT_DRC); }, "",
        nullptr,
        [is_mulity_menu]() {
            const Selection& selection = plater()->canvas3D()->get_selection();
            if (is_mulity_menu)
    ...
    if (!is_mulity_menu)
        return;
    append_menu_item(
        menu, wxID_ANY, _L("Export as DRCs") + dots, "", [](wxCommandEvent&) { plater()->export_stl(false, true, true, FT_DRC); }, "",
        nullptr,
        []() {
            const Selection& selection = plater()->canvas3D()->get_selection();
            return selection.is_multiple_full_instance() || selection.is_multiple_full_object();
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Reload from disk"), _L("Reload the selected parts from disk"),
        [](wxCommandEvent&) { plater()->reload_from_disk(); }, "", menu, []() { return plater()->can_reload_from_disk(); }, m_parent);
}

void MenuFactory::append_menu_item_replace_with_stl(wxMenu* menu)
{
    append_menu_item(
        menu, wxID_ANY, _L("Replace 3D file") + dots, _L("Replace the selected part with a new 3D file"),
        [](wxCommandEvent&) { plater()->replace_with_stl(); }, "", menu, []() { return plater()->can_replace_with_stl(); }, m_parent);
}

void MenuFactory::append_menu_item_replace_all_with_stl(wxMenu* menu)
{
    append_menu_item(
        menu, wxID_ANY, _L("Replace all with 3D files") + dots, _L("Replace all selected parts with 3D files from folder"),
        [](wxCommandEvent&) { plater()->replace_all_with_stl(); }, "", menu, []() { return plater()->can_replace_all_with_stl(); },
        m_parent);
}
...
void MenuFactory::append_menu_item_change_extruder(wxMenu* menu)
{
    ...
    for (int i = 0; i <= filaments_cnt; i++) {
        ...
        if (icon_idx >= 0 && icon_idx < icons.size()) {
            append_menu_item(
                extruder_selection_menu, wxID_ANY, item_name, "", [i](wxCommandEvent&) { obj_list()->set_extruder_for_selected_items(i); },
                *icons[icon_idx], menu, [is_active_extruder]() { return !is_active_extruder; }, m_parent);
        } else {
            append_menu_item(
                extruder_selection_menu, wxID_ANY, item_name, "", [i](wxCommandEvent&) { obj_list()->set_extruder_for_selected_items(i); },
                "", menu, [is_active_extruder]() { return !is_active_extruder; }, m_parent);
        }
    }
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Scale to build volume"), _L("Scale an object to fit the build volume"),
...
void MenuFactory::append_menu_items_flush_options(wxMenu* menu)
{
    ...
    append_menu_check_item(
        flush_options_menu, wxID_ANY, _L("Flush into objects' infill"), "",
    ...
    append_menu_check_item(
        flush_options_menu, wxID_ANY, _L("Flush into this object"), "",
        [&select_object_config, &global_config](wxCommandEvent&) {
    ...
    append_menu_check_item(
        flush_options_menu, wxID_ANY, _L("Flush into objects' support"), "",
        [&select_object_config, &global_config](wxCommandEvent&) {
...
void MenuFactory::append_menu_items_convert_unit(wxMenu* menu)
{
    ...
    std::vector<std::pair<ConversionType, wxString>> items = {{ConversionType::CONV_FROM_INCH, _L("Convert from inches")},
    ...
    for (auto item : items) {
        ...
        if (can_append(item.first)) {
            ...
            if (menu_id == wxNOT_FOUND)
                append_menu_item(
                    menu, wxID_ANY, item.second, item.second, [item](wxCommandEvent&) { plater()->convert_unit(item.first); }, "", menu,
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Assemble"), _L("Assemble the selected objects to an object with multiple parts"),
        [](wxCommandEvent&) { obj_list()->merge(true); }, "", menu, []() { return obj_list()->can_merge_to_multipart_object(); }, m_parent);
}
...
void MenuFactory::append_menu_item_merge_to_single_object(wxMenu* menu)
{
    menu->AppendSeparator();
    append_menu_item(
        menu, wxID_ANY, _L("Assemble"), _L("Assemble the selected objects to an object with single part"),
        [](wxCommandEvent&) { obj_list()->merge(false); }, "", menu, []() { return obj_list()->can_merge_to_single_object(); }, m_parent);
}
...
void MenuFactory::append_menu_item_merge_parts_to_single_part(wxMenu* menu)
{
    menu->AppendSeparator();
    append_menu_item(
        menu, wxID_ANY, _L("Mesh boolean"), _L("Mesh boolean operations including union and subtraction"),
        [](wxCommandEvent&) { obj_list()->boolean /*merge_volumes*/ (); }, "", menu, []() { return obj_list()->can_mesh_boolean(); },
        m_parent);
}
...
void MenuFactory::append_menu_items_mirror(wxMenu* menu)
{
    ...
    append_menu_item(
        mirror_menu, wxID_ANY, _L("Along X axis"), _L("Mirror along the X axis"), [](wxCommandEvent&) { plater()->mirror(X); },
        "menu_mirror_x", menu);
    append_menu_item(
        mirror_menu, wxID_ANY, _L("Along Y axis"), _L("Mirror along the Y axis"), [](wxCommandEvent&) { plater()->mirror(Y); },
        "menu_mirror_y", menu);
    append_menu_item(
        mirror_menu, wxID_ANY, _L("Along Z axis"), _L("Mirror along the Z axis"), [](wxCommandEvent&) { plater()->mirror(Z); },
        "menu_mirror_z", menu);

    append_submenu(menu, mirror_menu, wxID_ANY, _L("Mirror"), _L("Mirror object"), "", []() { return plater()->can_mirror(); }, m_parent);
}
...
void MenuFactory::append_menu_item_invalidate_cut_info(wxMenu* menu)
{
    ...
    if (obj_list()->has_selected_cut_object())
        append_menu_item(
            menu, wxID_ANY, menu_name, "", [](wxCommandEvent&) { obj_list()->invalidate_cut_info_for_selection(); }, "", menu,
            []() { return true; }, m_parent);
}
...
void MenuFactory::create_default_menu()
{
...
#ifdef __WINDOWS__
    append_submenu(&m_default_menu, sub_menu_primitives, wxID_ANY, _L("Add Primitive"), "", "menu_add_part", []() { return true; }, m_parent);
    append_submenu(&m_default_menu, sub_menu_handy, wxID_ANY, _L("Add Handy models"), "", "menu_add_part", []() { return true; }, m_parent);
    append_menu_item(
        &m_default_menu, wxID_ANY, _L("Add Models"), "", // ORCA: Add Models
        [](wxCommandEvent&) { plater()->add_file(); }, "menu_add_part", &m_default_menu,
        []() { return wxGetApp().plater()->can_add_model(); }, m_parent);
#else
    append_submenu(&m_default_menu, sub_menu_primitives, wxID_ANY, _L("Add Primitive"), "", "", []() { return true; }, m_parent);
    append_submenu(&m_default_menu, sub_menu_handy, wxID_ANY, _L("Add Handy models"), "", "", []() { return true; }, m_parent);
    append_menu_item(
        &m_default_menu, wxID_ANY, _L("Add Models"), "", // ORCA: Add Models
        [](wxCommandEvent&) { plater()->add_file(); }, "", &m_default_menu, []() { return wxGetApp().plater()->can_add_model(); },
        m_parent);
#endif
    ...
    append_menu_check_item(
        &m_default_menu, wxID_ANY, _L("Show Labels"), "",
        [](wxCommandEvent&) {
            plater()->show_view3D_labels(!plater()->are_view3D_labels_shown());
            plater()->get_current_canvas3D()->post_event(SimpleEvent(wxEVT_PAINT));
        },
        &m_default_menu, []() { return plater()->is_view3D_shown(); }, [this]() { return plater()->are_view3D_labels_shown(); }, m_parent);
}
...
void MenuFactory::create_object_menu()
{
    ...
    append_menu_item(
        split_menu, wxID_ANY, _L("To objects"), _L("Split the selected object into multiple objects"),
        [](wxCommandEvent&) { plater()->split_object(); }, "menu_split_objects", &m_object_menu, []() { return plater()->can_split(true); },
        m_parent);
    append_menu_item(
        split_menu, wxID_ANY, _L("To parts"), _L("Split the selected object into multiple parts"),
        [](wxCommandEvent&) { plater()->split_volume(); }, "menu_split_parts", &m_object_menu, []() { return plater()->can_split(false); },
        m_parent);

    append_submenu(
        &m_object_menu, split_menu, wxID_ANY, _L("Split"), _L("Split the selected object"), "",
...
void MenuFactory::create_extra_object_menu()
{
    ...
    append_submenu(
        &m_object_menu, split_menu, wxID_ANY, _L("Split"), _L("Split the selected object"), "", []() { return plater()->can_split(true); },
        m_parent);
...
void MenuFactory::create_sla_object_menu()
{
    ...
    append_menu_item(
        &m_sla_object_menu, wxID_ANY, _L("Split"), _L("Split the selected object into multiple objects"),
        [](wxCommandEvent&) { plater()->split_object(); }, "", nullptr, []() { return plater()->can_split(true); }, m_parent);
...
void MenuFactory::create_part_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Split"), _L("Split the selected object into multiple parts"), [](wxCommandEvent&) { plater()->split_volume(); },
        "split_parts", nullptr, []() { return plater()->can_split(false); }, m_parent);

    menu->AppendSeparator();
    append_menu_item_change_type(menu);
    append_menu_items_mirror(&m_part_menu);
    append_menu_item(
        &m_part_menu, wxID_ANY, _L("Split"), _L("Split the selected object into multiple parts"),
        [](wxCommandEvent&) { plater()->split_volume(); }, "split_parts", nullptr, []() { return plater()->can_split(false); }, m_parent);
...
void MenuFactory::create_bbl_part_menu()
{
    ...
    append_menu_item(
        split_menu, wxID_ANY, _L("To objects"), _L("Split the selected object into multiple objects"),
        [](wxCommandEvent&) { plater()->split_object(); }, "menu_split_objects", menu, []() { return plater()->can_split(true); },
        m_parent);
    append_menu_item(
        split_menu, wxID_ANY, _L("To parts"), _L("Split the selected object into multiple parts"),
        [](wxCommandEvent&) { plater()->split_volume(); }, "menu_split_parts", menu, []() { return plater()->can_split(false); }, m_parent);

    append_submenu(
        menu, split_menu, wxID_ANY, _L("Split"), _L("Split the selected object"), "", []() { return plater()->can_split(true); }, m_parent);
    menu->AppendSeparator();
    append_menu_item_per_object_process(menu);
    append_menu_item_per_object_settings(menu);
    append_menu_item_change_type(menu);
...
void MenuFactory::create_plate_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Select All"), _L("Select all objects on the current plate"),
        [](wxCommandEvent&) { plater()->select_curr_plate_all(); }, "", nullptr,
        []() {
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Select All Plates"), _L("Select all objects on all plates"), [](wxCommandEvent&) { plater()->select_all(); },
        "", nullptr, []() { return !plater()->model().objects.empty(); }, m_parent);

    // delete objects on current plate
    append_menu_item(
        menu, wxID_ANY, _L("Delete All"), _L("Delete all objects on the current plate"),
        [](wxCommandEvent&) { plater()->remove_curr_plate_all(); }, "", nullptr,
        []() {
            PartPlate* plate = plater()->get_partplate_list().get_selected_plate();
            assert(plate);
            return !plate->get_objects().empty();
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Arrange"), _L("Arrange current plate"),
        [](wxCommandEvent&) {
            PartPlate* plate = plater()->get_partplate_list().get_selected_plate();
            assert(plate);
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Auto Rotate"), _L("Auto rotate current plate"),
        [](wxCommandEvent&) {
            PartPlate* plate = plater()->get_partplate_list().get_selected_plate();
            assert(plate);
...
#ifdef __WINDOWS__
    append_menu_item(
        menu, wxID_ANY, _L("Delete Plate"), _L("Remove the selected plate"), [](wxCommandEvent&) { plater()->delete_plate(); },
        "menu_delete", nullptr, []() { return plater()->can_delete_plate(); }, m_parent);
#else
    append_menu_item(
        menu, wxID_ANY, _L("Delete Plate"), _L("Remove the selected plate"), [](wxCommandEvent&) { plater()->delete_plate(); }, "", nullptr,
...
#ifdef __WINDOWS__
    append_submenu(menu, sub_menu_primitives, wxID_ANY, _L("Add Primitive"), "", "menu_add_part", []() { return true; }, m_parent);
    append_submenu(menu, sub_menu_handy, wxID_ANY, _L("Add Handy models"), "", "menu_add_part", []() { return true; }, m_parent);
    append_menu_item(
        menu, wxID_ANY, _L("Add Models"), "", // ORCA: Add Models
        [](wxCommandEvent&) { plater()->add_file(); }, "menu_add_part", menu, []() { return wxGetApp().plater()->can_add_model(); },
        m_parent);
#else
    append_submenu(menu, sub_menu_primitives, wxID_ANY, _L("Add Primitive"), "", "", []() { return true; }, m_parent);
    append_submenu(menu, sub_menu_handy, wxID_ANY, _L("Add Handy models"), "", "", []() { return true; }, m_parent);
    append_menu_item(
        menu, wxID_ANY, _L("Add Models"), "", // ORCA: Add Models
        [](wxCommandEvent&) { plater()->add_file(); }, "", menu, []() { return wxGetApp().plater()->can_add_model(); }, m_parent);
#endif
    append_menu_item_replace_all_with_stl(menu);
...
wxMenu* MenuFactory::default_menu() { return &m_default_menu; }
...
wxMenu* MenuFactory::instance_menu() { return &m_instance_menu; }
...
wxMenu* MenuFactory::multi_selection_menu()
{
    ...
    if (!multi_volume) {
        ...
        if (split_menu) {
            ...
            append_menu_item(
                split_menu, wxID_ANY, _L("To parts"), _L("Split the selected object into multiple parts"),
                [](wxCommandEvent&) { plater()->split_volume(); }, "menu_split_parts", menu, []() { return plater()->can_split(false); },
                m_parent);

            append_submenu(
                menu, split_menu, wxID_ANY, _L("Split"), _L("Split the selected object"), "", []() { return plater()->can_split(true); },
                m_parent);
        }
        append_menu_item_per_object_process(menu);
        menu->AppendSeparator();
...
void MenuFactory::append_menu_items_instance_manipulation(wxMenu* menu)
{
    ...
    items_increase[type] = append_menu_item(
        menu, wxID_ANY, _L("Add instance") + "\t+", _L("Add one more instance of the selected object"),
        [](wxCommandEvent&) { plater()->increase_instances(); }, "", nullptr, []() { return plater()->can_increase_instances(); },
        m_parent);
    items_decrease[type] = append_menu_item(
        menu, wxID_ANY, _L("Remove instance") + "\t-", _L("Remove one instance of the selected object"),
        [](wxCommandEvent&) { plater()->decrease_instances(); }, "", nullptr, []() { return plater()->can_decrease_instances(); },
        m_parent);
    items_set_number_of_copies[type] = append_menu_item(
        ...
        [](wxCommandEvent&) { plater()->set_number_of_copies(); }, "", nullptr, []() { return plater()->can_increase_instances(); },
        m_parent);
    append_menu_item(
        menu, wxID_ANY, _L("Fill bed with instances") + dots, _L("Fill the remaining area of bed with instances of the selected object"),
        [](wxCommandEvent&) { plater()->fill_bed_with_instances(); }, "", nullptr, []() { return plater()->can_increase_instances(); },
        m_parent);
}
...
#endif
    append_menu_item(
        menu, wxID_ANY, _L("Clone") + "\t" + ctrl + "K", "", [this](wxCommandEvent&) { plater()->clone_selection(); }, "", nullptr,
        []() { return true; }, m_parent);
}
...
{
    wxMenuItem* menu_item = append_menu_item(
        menu, wxID_ANY, _L("Simplify Model"), "", [](wxCommandEvent&) { obj_list()->simplify(); }, "", menu,
...
{
    wxMenuItem* menu_item = append_menu_item(
        menu, wxID_ANY, _L("Subdivision mesh") + _L("(Lost color)"), "", [](wxCommandEvent&) { obj_list()->smooth_mesh(); }, "", menu,
        []() { return plater()->can_smooth_mesh(); }, m_parent);
}

void MenuFactory::append_menu_item_center(wxMenu* menu)
{
    append_menu_item(
        menu, wxID_ANY, _L("Center"), "", [this](wxCommandEvent&) { plater()->center_selection(); }, "", nullptr,
        []() {
...
{
    append_menu_item(
        menu, wxID_ANY, _L("Drop"), "", [this](wxCommandEvent&) { plater()->drop_selection(); }, "", nullptr,
        []() {
            if (plater()->canvas3D()->get_canvas_type() != GLCanvas3D::ECanvasType::CanvasView3D)
                return false;
            else {
...
void MenuFactory::append_menu_item_per_object_process(wxMenu* menu)
{
    ...
    append_menu_item(
        menu, wxID_ANY, names[0], names[1], [](wxCommandEvent&) { wxGetApp().obj_list()->switch_to_object_process(); }, "", nullptr,
        []() {
            Selection& selection = plater()->canvas3D()->get_selection();
            return selection.is_single_full_object() || selection.is_multiple_full_object() || selection.is_single_full_instance() ||
                   selection.is_multiple_full_instance() || selection.is_single_volume() || selection.is_multiple_volume();
        },
        m_parent);

    const std::vector<wxString> names2 = {_L("Copy Process Settings"), _L("Copy Process Settings")};
    append_menu_item(
        menu, wxID_ANY, names2[0], names2[1], [](wxCommandEvent&) { wxGetApp().obj_list()->copy_settings_to_clipboard(); }, "", nullptr,
        []() {
            Selection& selection = plater()->canvas3D()->get_selection();
            return selection.is_single_full_object() || selection.is_single_full_instance() || selection.is_single_volume_or_modifier();
        },
        m_parent);

    const std::vector<wxString> names3 = {_L("Paste Process Settings"), _L("Paste Process Settings")};
    append_menu_item(
        menu, wxID_ANY, names3[0], names3[1], [](wxCommandEvent&) { wxGetApp().obj_list()->paste_settings_into_list(); }, "", nullptr,
        []() { return wxGetApp().obj_list()->can_paste_settings_into_list(); }, m_parent);
}
...
void MenuFactory::append_menu_item_per_object_settings(wxMenu* menu)
{
    ...
    append_menu_item(
        menu, wxID_ANY, names[0], names[1], [](wxCommandEvent&) { plater()->PopupObjectTableBySelection(); }, "", nullptr,
        []() {
            Selection& selection = plater()->canvas3D()->get_selection();
...
void MenuFactory::append_menu_item_change_filament(wxMenu* menu)
{
    ...
    for (int i = has_modifier ? 0 : 1; i <= filaments_cnt; i++) {
        ...
        append_menu_item(
            extruder_selection_menu, wxID_ANY, item_name, "", [i](wxCommandEvent&) { obj_list()->set_extruder_for_selected_items(i); },
            i == 0 ? wxNullBitmap : *icons[i - 1], menu, [is_active_extruder]() { return !is_active_extruder; }, m_parent);
    }
...
void MenuFactory::append_menu_item_set_printable(wxMenu* menu)
{
    ...
    wxMenuItem* menu_item_set_printable = append_menu_check_item(
        menu, wxID_ANY, menu_text, "",
        [this, all_printable](wxCommandEvent&) {
            Selection& selection = plater()->canvas3D()->get_selection();
    ...
    m_parent->Bind(
        wxEVT_UPDATE_UI,
        [all_printable](wxUpdateUIEvent& evt) {
...
void MenuFactory::update_object_menu() { append_menu_items_add_volume(&m_object_menu); }
...
template<typename T> class menu_has_update_def_colors
{
    typedef char one;
    struct two
    {
...
template<typename T> static void update_menu_item_def_colors(T* item)
{
