...
boost::any ConfigOptionsGroup::get_config_value(const DynamicPrintConfig& config, const std::string& opt_key, int opt_index /*= -1*/)
{
    ...
    std::unique_ptr<ConfigOption> default_opt;

    if (opt == nullptr)
        return ret;

    const ConfigOption* config_opt = config.option(opt_key);
    if (config_opt == nullptr && opt->default_value) {
        default_opt.reset(opt->create_default_option());
        config_opt = default_opt.get();
    }
    ...
    if (opt->nullable) {
        ...
        case coPercents:
        case coFloats: {
            if (opt_index < 0 ? config_opt->is_nil() : dynamic_cast<ConfigOptionVectorBase const*>(config_opt)->is_nil(opt_index))
                ret = _(L("N/A"));
            else {
                double val = opt->type == coFloats ? static_cast<const ConfigOptionFloatsNullable*>(config_opt)->get_at(idx) :
                                                     static_cast<const ConfigOptionPercentsNullable*>(config_opt)->get_at(idx);
        ...
        case coFloatsOrPercents: {
            if (opt_index < 0 ? config_opt->is_nil() : dynamic_cast<ConfigOptionVectorBase const*>(config_opt)->is_nil(opt_index))
                ...
                const auto& value = static_cast<const ConfigOptionFloatsOrPercentsNullable*>(config_opt)->get_at(idx);
        ...
        case coBools: ret = static_cast<const ConfigOptionBoolsNullable*>(config_opt)->values[idx]; break;
        case coInts: ret = static_cast<const ConfigOptionIntsNullable*>(config_opt)->get_at(idx); break;
        case coEnums: ret = static_cast<const ConfigOptionEnumsGenericNullable*>(config_opt)->get_at(idx); break;
        default: break;
        }
        return ret;
    }
    ...
    switch (opt->type) {
    case coFloatOrPercent: {
        const auto& value = *static_cast<const ConfigOptionFloatOrPercent*>(config_opt);
    ...
    case coFloatsOrPercents: {
        const auto& value = static_cast<const ConfigOptionFloatsOrPercents*>(config_opt)->get_at(idx);
    ...
    case coPercent: {
        double val = static_cast<const ConfigOptionPercent*>(config_opt)->value;
        ret        = double_to_string(val); // += "%";
    } break;
    case coPercents:
    case coFloats:
    case coFloat: {
        if (opt_key == "extruder_printable_height") {
            auto opt_values = dynamic_cast<const ConfigOptionFloatsNullable*>(config_opt)->values;
            if (!opt_values.empty()) {
                double val = opt_values[idx];
                ret        = double_to_string(val);
            }
        } else {
            double val = opt->type == coFloats ? static_cast<const ConfigOptionFloats*>(config_opt)->get_at(idx) :
                         opt->type == coFloat  ? static_cast<const ConfigOptionFloat*>(config_opt)->value :
                                                 static_cast<const ConfigOptionPercents*>(config_opt)->get_at(idx);
            ret        = double_to_string(val);
        }
        break;
    }
    case coString: ret = from_u8(static_cast<const ConfigOptionString*>(config_opt)->value); break;
    case coStrings:
        if (opt_key == "compatible_printers" || opt_key == "compatible_prints") {
            ret = static_cast<const ConfigOptionStrings*>(config_opt)->values;
            break;
        }
        if (static_cast<const ConfigOptionStrings*>(config_opt)->values.empty())
            ret = text_value;
        else if (opt->gui_flags == "serialized") {
            std::vector<std::string> values = static_cast<const ConfigOptionStrings*>(config_opt)->values;
            if (!values.empty() && !values[0].empty())
                for (auto el : values)
                    text_value += el + ";";
            ret = text_value;
        } else
            ret = from_u8(static_cast<const ConfigOptionStrings*>(config_opt)->get_at(idx));
        break;
    case coBool: ret = static_cast<const ConfigOptionBool*>(config_opt)->value; break;
    case coBools: ret = static_cast<const ConfigOptionBools*>(config_opt)->get_at(idx); break;
    case coInt: ret = static_cast<const ConfigOptionInt*>(config_opt)->value; break;
    case coInts: ret = static_cast<const ConfigOptionInts*>(config_opt)->get_at(idx); break;
    case coEnum:
        ...
        ret = config_opt->getInt();
        break;
    // BBS
    case coEnums: ret = static_cast<const ConfigOptionEnumsGeneric*>(config_opt)->get_at(idx); break;
    case coPoint: ret = static_cast<const ConfigOptionPoint*>(config_opt)->value; break;
    case coPoints:
        if (opt_key == "printable_area")
            ret = get_thumbnails_string(static_cast<const ConfigOptionPoints*>(config_opt)->values);
        else if (opt_key == "bed_exclude_area")
            ret = get_thumbnails_string(static_cast<const ConfigOptionPoints*>(config_opt)->values);
        else if (opt_key == "wrapping_exclude_area")
            ret = get_thumbnails_string(static_cast<const ConfigOptionPoints*>(config_opt)->values);
        else
            ret = static_cast<const ConfigOptionPoints*>(config_opt)->get_at(idx);
        break;
    case coPointsGroups:
        if (opt_key == "extruder_printable_area") {
            auto values = static_cast<const ConfigOptionPointsGroups*>(config_opt)->values;
            if (!values.empty())
                ret = get_thumbnails_string(static_cast<const ConfigOptionPointsGroups*>(config_opt)->get_at(idx));
        } else
            ret = static_cast<const ConfigOptionPointsGroups*>(config_opt)->get_at(idx);
        break;
    case coNone:
    default: break;
    }
