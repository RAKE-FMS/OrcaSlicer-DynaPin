...
{
    return config.origin_y + double(pin.row - config.origin_row) * config.row_pitch_y;
}
...
bool load_config_for_print(const Print &print, Config &config, std::string *error)
{
    ...
    const nlohmann::json exclusion = j.contains("support_exclusion") && j["support_exclusion"].is_object() ?
                                         j["support_exclusion"] :
...
std::vector<Polygons> support_blockers_for_object(const PrintObject &object)
{
    ...
    Config config;
    std::string error;
    if (!load_config_for_print(*object.print(), config, &error)) {
        BOOST_LOG_TRIVIAL(warning) << error;
        return out;
    }

    const std::vector<Pin> pins = parse_pin_list(object.config().dynapin_selected_pins.value);
...
std::string pull_gcode_for_pin(const Config &config, const Pin &pin)
{
    ...
    gcode << "; END_DYNAPIN_PULL row=" << pin.row << " col=" << pin.col << "\n";
    return gcode.str();
}
