...
std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_filament_id, double z) const
{
    ...
    if (is_nozzle_change) {
        ...
        start_pos_str = gcodegen.travel_to(wipe_tower_point_to_object_point(gcodegen, transform_wt_pt(tcr.nozzle_change_result.start_pos) +
                                                                                          plate_origin_2d),
                                           erMixed, "Move to nozzle change start pos");
        check_add_eol(start_pos_str);
        nozzle_change_gcode_trans += start_pos_str;
        nozzle_change_gcode_trans += gcodegen.unretract();
        nozzle_change_gcode_trans += transform_gcode(tcr.nozzle_change_result.gcode, tcr.nozzle_change_result.start_pos, wipe_tower_offset,
                                                     wipe_tower_rotation);
        gcodegen.set_last_pos(
            wipe_tower_point_to_object_point(gcodegen, transform_wt_pt(tcr.nozzle_change_result.end_pos) + plate_origin_2d));
    ...
    bool        need_travel_after_change_filament_gcode =
    ...
    if (!change_filament_gcode.empty()) {
        ...
        // BBS
        {
            ...
            float outer_wall_volumetric_speed = get_outer_wall_volumetric_speed(full_config, *gcodegen.m_print, new_filament_id,
                                                                                gcodegen.get_extruder_id(new_filament_id));
            ...
            float old_retract_length_toolchange = (old_filament_id != -1) ? full_config.retract_length_toolchange.get_at(old_filament_id) :
                                                                            0;
            float new_retract_length_toolchange = full_config.retract_length_toolchange.get_at(new_filament_id);
            int old_filament_temp = (old_filament_id != -1) ?
                                        (gcodegen.on_first_layer() ? full_config.nozzle_temperature_initial_layer.get_at(old_filament_id) :
                                                                     full_config.nozzle_temperature.get_at(old_filament_id)) :
                                        210;
            int new_filament_temp = gcodegen.on_first_layer() ? full_config.nozzle_temperature_initial_layer.get_at(new_filament_id) :
            ...
            int old_filament_e_feedrate = (old_filament_id != -1) ?
                                              (int) (60.0 * full_config.filament_max_volumetric_speed.get_at(old_filament_id) /
                                                     filament_area) :
                                              200;
            ...
            config.set_key_value("filament_tower_interface_purge_volume",
                                 new ConfigOptionFloat(full_config.filament_tower_interface_purge_volume.get_at(new_filament_id)));
    ...
    gcodegen.placeholder_parser().set("retraction_distance_when_cut",
                                      gcodegen.m_config.retraction_distances_when_cut.get_at(new_filament_id));
    ...
    if (!filament_start_gcode.empty()) {
        ...
        start_filament_gcode_str = gcodegen.placeholder_parser_process("filament_start_gcode", filament_start_gcode, new_filament_id,
                                                                       &config);
    ...
    std::string tcr_gcode,
        tcr_escaped_gcode = gcodegen.placeholder_parser_process("tcr_rotated_gcode", tcr_rotated_gcode, new_filament_id, &config);
...
std::string WipeTowerIntegration::append_tcr2(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_extruder_id, double z) const
{
    ...
    if (tcr.priming || (new_extruder_id >= 0 && needs_toolchange)) {
        ...
        toolchange_gcode_str = gcodegen.set_extruder(new_extruder_id, tcr.print_z, false,
                                                     toolchange_temp_override); // TODO: toolchange_z vs print_z
            ...
            if (boost::starts_with(trimmed, "M109")) {
                ...
                if (!matches_extruder) {
                    ...
                    if (t_pos != std::string::npos) {
                        ...
                        const std::string t_val = trimmed.substr(t_pos + 1,
                                                                 t_end == std::string::npos ? std::string::npos : t_end - (t_pos + 1));
                        if (!t_val.empty()) {
                            try {
                ...
                if (matches_extruder) {
                    ...
                    if (s_pos != std::string::npos) {
                        ...
                        const std::string s_val = trimmed.substr(s_pos + 1,
                                                                 s_end == std::string::npos ? std::string::npos : s_end - (s_pos + 1));
                        if (!s_val.empty()) {
                            try {
    ...
    if (toolchange_temp_override > 0) {
        ...
        while (pos < tcr_rotated_gcode.size()) {
            ...
            if (is_preheat_line) {
                ...
                if (s_pos != std::string::npos) {
                    size_t s_end = trimmed.find_first_not_of("0123456789", s_pos + 1);
                    trimmed.replace(s_pos + 1, (s_end == std::string::npos ? trimmed.size() : s_end) - (s_pos + 1),
        ...
        if (layers_to_print.size() == 1u) {
            if (!has_extrusions)
                throw Slic3r::SlicingError(
                    _(L("One object has an empty first layer and can't be printed. Please Cut the bottom or enable supports.")),
                    object.id().id);
        }
...
static std::vector<std::pair<std::string, std::string>> validate_custom_gcode(const Print& print)
{
    ...
    if (ret.size() < MAX_TAGS_COUNT)
        check(_(L("Timelapse G-code")), config.wrapping_detection_gcode.value);
    if (ret.size() < MAX_TAGS_COUNT)
...
void GCode::do_export(Print* print, const char* path, GCodeProcessorResult* result, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    for (const auto& filament : m_writer.extruders())
        activate_long_retraction_when_cut |= (m_config.long_retractions_when_cut.get_at(filament.id()) &&
    ...
    { // BBS:check bed and filament compatible
        ...
        m_processor.result().filament_printable_reuslt = FilamentPrintableResult(conflict_filament,
                                                                                 bed_type_to_gcode_string(m_config.curr_bed_type));
    }
    ...
    m_processor.check_multi_extruder_gcode_valid(extruder_size, plate_printable_area, m_print->config().printable_height.value,
                                                 wrapping_exclude_area_points, extruder_unprintable_polys,
                                                 m_print->get_extruder_printable_height(), m_print->get_filament_maps(),
...
void GCode::_do_export(Print& print, GCodeOutputStream& file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    // Process file_start_gcode - written at the very top of the file, before any header
    {
        ...
        if (!top_gcode_template.empty()) {
            ...
            top_config.set_key_value("print_time_sec", new ConfigOptionString(GCodeProcessor::reserved_tag(
                                                           GCodeProcessor::ETags::Print_Time_Sec_Placeholder)));
            top_config.set_key_value("used_filament_length", new ConfigOptionString(GCodeProcessor::reserved_tag(
                                                                 GCodeProcessor::ETags::Used_Filament_Length_Placeholder)));
    ...
    if (print.config().print_sequence == PrintSequence::ByObject) {
        ...
        for (; print_object_instance_sequential_active != print_object_instances_ordering.end(); ++print_object_instance_sequential_active) {
            ...
            if (!find_fist_non_support_filament && tool_ordering.first_extruder() != (unsigned int) -1) {
                ...
                find_fist_non_support_filament = tool_ordering.cal_non_support_filaments(print.config(), initial_non_support_extruder_id,
                                                                                         first_non_support_filaments, first_filaments);
            }
        }
        ...
        initial_extruder_id = (wipe_tower_type == WipeTowerType::Type2 && has_wipe_tower &&
...

    {
...

        {
            ...
            for (auto& obj : print.objects()) {
                max_print_z = std::max(max_print_z, (*std::max_element(obj->layers().begin(), obj->layers().end(), [](Layer* a, Layer* b) {
    ...
    }
    {
        ...
        for (const auto& extruder : m_writer.extruders())
            min_temperature_vitrification = std::min(min_temperature_vitrification,
                                                     m_config.temperature_vitrification.get_at(extruder.id()));
        ...
        bool has_tpu_in_first_layer = std::any_of(first_layer_filaments.begin(), first_layer_filaments.end(),
                                                  [&](unsigned int idx) { return m_config.filament_type.values[idx] == "TPU"; });
    ...
    {                                                                           // hold chamber temp for flat print: Flag
        ...
        for (ObjectID print_object_ID_t : print.print_object_ids()) {
            ...
            // object hight
            if (!print_object->layers().empty() && print_object->layers().back()->print_z > max_hight_temp)
                max_hight_temp = print_object->layers().back()->print_z;
            // object area
            if (!print_object->layers().empty() &&
                print_object->layers().front()->print_z < print.config().initial_layer_print_height + EPSILON &&
                !print_object->layers().front()->lslices.empty()) {
                ExPolygons temp_Expolys = print_object->layers().front()->lslices;
                for (ExPolygon& temp_Expoly : temp_Expolys) {
                    area_sum_temp += temp_Expoly.area();
                }
            }
            // suport area
            if (!print_object->support_layers().empty() &&
                print_object->support_layers().front()->print_z < print.config().initial_layer_print_height + EPSILON &&
            ...
            // brim area
            if (print.m_brimMap.find(print_object_ID_t) != print.m_brimMap.end() &&
                !print.m_brimMap.at(print_object_ID_t).entities.empty()) { // contain brim
                for (const ExtrusionEntity* entities_temp : print.m_brimMap.at(print_object_ID_t).entities) {
                    ...
                    for (Polygon& temp_Expoly : temp_Expolys) {
                        area_sum_temp += temp_Expoly.area();
                    }
                }
            }
        ...
        bool hold_chamber_temp_for_flat_print = max_hight_temp > 0 && max_hight_temp < pring_hight_threshold &&
                                                area_sum_temp > print_area_sum_threshold * 1.0e10;
        this->placeholder_parser().set("hold_chamber_temp_for_flat_print", new ConfigOptionBool(hold_chamber_temp_for_flat_print));
    }

    this->placeholder_parser().set("print_time_sec",
                                   new ConfigOptionString(GCodeProcessor::reserved_tag(GCodeProcessor::ETags::Print_Time_Sec_Placeholder)));
    this->placeholder_parser().set("used_filament_length", new ConfigOptionString(GCodeProcessor::reserved_tag(
                                                               GCodeProcessor::ETags::Used_Filament_Length_Placeholder)));
    ...
    if (is_bbl_printers) {
        ...
        // add the missing filament start gcode in machine start gcode
        {
            ...
            std::string filament_start_gcode = this->placeholder_parser_process("filament_start_gcode",
                                                                                print.config().filament_start_gcode.values.at(
 ...
 */
        ...
        if (print.config().print_sequence == PrintSequence::ByObject && !has_wipe_tower) {
            ...
            for (; print_object_instance_sequential_active != print_object_instances_ordering.end();
                                     ...
                                     *print_object_instance_sequential_active - object.instances().data(), file, prime_extruder);
                {
                    ...
                    print.m_statistics_by_extruder_count.stats_by_single_extruder += tool_ordering.get_filament_change_stats(
                        ToolOrdering::FilamentChangeMode::SingleExt);
                    print.m_statistics_by_extruder_count.stats_by_multi_extruder_best += tool_ordering.get_filament_change_stats(
                        ToolOrdering::FilamentChangeMode::MultiExtBest);
                    print.m_statistics_by_extruder_count.stats_by_multi_extruder_curr += tool_ordering.get_filament_change_stats(
                        ToolOrdering::FilamentChangeMode::MultiExtCurr);
    ...
    // Process filament-specific gcode in extruder order.
    {
        ...
        file.writeln(
            this->placeholder_parser_process("machine_end_gcode", print.config().machine_end_gcode, m_writer.filament()->id(), &config));
    }
                           ...
                           GCodeOutputStream&                                                 output_stream)
{
         ...
         &layer_to_print_idx](tbb::flow_control& fc) -> LayerResult {
            if (layer_to_print_idx >= layers_to_print.size()) {
                ...
                return this->process_layer(print, layer.second, layer_tools, &layer == &layers_to_print.back(),
                                           &print_object_instances_ordering, tool_ordering.get_most_used_extruder(), size_t(-1));
            }
        });
    if (m_spiral_vase) {
    ...
    const auto spiral_mode         = tbb::make_filter<LayerResult, LayerResult>(slic3r_tbb_filtermode::serial_in_order,
                                                                                [&spiral_mode = *this->m_spiral_vase.get(),
                                                                                 &layers_to_print](LayerResult in) -> LayerResult {
                                                                            if (in.nop_layer_result)
                                                                                return in;

                                                                            spiral_mode.enable(in.spiral_vase_enable);
                                                                            bool last_layer = in.layer_id == layers_to_print.size() - 1;
                                                                            return {spiral_mode.process_layer(std::move(in.gcode),
                                                                                                              last_layer),
                                                                                    in.layer_id, in.spiral_vase_enable,
                                             ...
                                             prime_extruder](tbb::flow_control& fc) -> LayerResult {
                                                if (layer_to_print_idx >= layers_to_print.size()) {
                                                    ...
                                                    return this->process_layer(print, {std::move(layer)},
                                                                               ...
                                                                               tool_ordering.get_most_used_extruder(), single_object_idx,
                                                                               prime_extruder);
                                                }
                                            });
    if (m_spiral_vase) {
...
std::string GCode::placeholder_parser_process(const std::string&   name,
                                              ...
                                              unsigned int         current_filament_id,
                                 ...
                                 const bool prime_extruder)
{
    ...
    if (!m_config.layer_change_gcode.value.empty()) {
        ...
        config.set_key_value("most_used_physical_extruder_id",
                             new ConfigOptionInt(m_config.physical_extruder_map.get_at(most_used_extruder)));
        config.set_key_value("layer_num", new ConfigOptionInt(m_layer_index));
        config.set_key_value("layer_z", new ConfigOptionFloat(print_z));
        gcode += this->placeholder_parser_process("layer_change_gcode", print.config().layer_change_gcode.value, m_writer.filament()->id(),
                                                  &config) +
                 "\n";
        config.set_key_value("max_layer_z", new ConfigOptionFloat(m_max_layer_z));
    }
        ...
        if (m_layer_index == 1) {
            ...
            if (print.calib_params().freqStartX == print.calib_params().freqStartY &&
                ...
                gcode += writer().set_input_shaping('A', 0.f,
                                                    this->interpolate_value_across_layers(print.calib_params().freqStartX,
                                                                                          print.calib_params().freqEndX),
                                                    "");
            } else {
                ...
                gcode += writer().set_input_shaping('Y', 0.f,
                                                    this->interpolate_value_across_layers(print.calib_params().freqStartY,
                                                                                          print.calib_params().freqEndY),
                                                    "");
            }
        }
        break;
    ...
    case CalibMode::Calib_Input_shaping_damp: {
        if (m_layer_index == 1) {
            ...
            gcode += writer().set_input_shaping('A',
                                                this->interpolate_value_across_layers(print.calib_params().start, print.calib_params().end),
                                                0.f, "");
        }
        break;
    }
    case CalibMode::Calib_Cornering: {
        if (m_writer.get_gcode_flavor() == gcfMarlinFirmware && !m_config.machine_max_junction_deviation.values.empty() &&
            m_config.machine_max_junction_deviation.values.front() > 0) {
            gcode += writer().set_junction_deviation(
                this->interpolate_value_across_layers(print.calib_params().start, print.calib_params().end));
        } else {
    ...
    if (single_object_instance_idx == size_t(-1)) {
        ...
        gcode += ProcessLayer::emit_custom_gcode_per_print_z(*this, layer_tools.custom_gcode, m_writer.filament()->id(), first_extruder_id,
                                                             print.config());
    }
    ...
    auto get_next_extruder = [&](int current_extruder, const std::vector<unsigned int>& extruders) {
        std::vector<float> flush_matrix(
            cast<float>(get_flush_volumes_matrix(m_config.flush_volumes_matrix.values, 0, m_config.nozzle_diameter.values.size())));
    ...
    std::map<unsigned int, std::vector<InstanceToPrint>> filament_to_print_instances;
    {
        for (unsigned int filament_id : layer_tools.extruders) {
            ...
            if (print.config().print_sequence == PrintSequence::ByObject) {
                filament_to_print_instances[filament_id] = sort_print_object_instances(objects_by_extruder_it->second, layers, ordering,
                                                                                       single_object_instance_idx);
            } else {
                // PrintSequence::ByLayer to use global ordering ( per object ordering ) if intra-layer order PrintOrder::AsObjectList is
                // specified while keeping behaviour of PrintSequence::ByLayer
                const std::vector<const PrintInstance*>* ordering_for_filament = (print.config().print_order == PrintOrder::AsObjectList &&
                                                                                  ordering != nullptr) ?
                                                                                     ordering :
                                                                                     &new_ordering;
                filament_to_print_instances[filament_id] = sort_print_object_instances(objects_by_extruder_it->second, layers,
                                                                                       ordering_for_filament, single_object_instance_idx);
            }
        }
    }
    ...
    auto insert_timelapse_gcode = [this, print_z, &print, &most_used_extruder, &layer_object_label_ids,
                                   &printed_objects = std::as_const(m_printed_objects)]() -> std::string {
        PosPickCtx ctx;
        ...
        if (!print.config().time_lapse_gcode.value.empty()) {
            ...
            config.set_key_value("curr_physical_extruder_id",
                                 new ConfigOptionInt(m_config.physical_extruder_map.get_at(ctx.curr_extruder_id)));
            ...
            timelapse_gcode = this->placeholder_parser_process("timelapse_gcode", print.config().time_lapse_gcode.value,
                                                               m_writer.filament()->id(), &config) +
                              "\n";
        }
        ...
        if (is_BBL_Printer() && (print.num_object_instances() <= g_max_label_object) && // Don't support too many objects on one plate
            (print.num_object_instances() > 1) &&                                       // Don't support skipping single object
            (layer_object_label_ids.size() > 0) && (print.calib_params().mode == CalibMode::Calib_None)) {
            std::ostringstream oss;
            for (auto it = layer_object_label_ids.begin(); it != layer_object_label_ids.end(); ++it) {
                if (it != layer_object_label_ids.begin())
                    oss << ",";
                oss << *it;
            }
            ...
            std::string start_str = std::string("; object ids of layer ") + std::to_string(m_layer_index + 1) + (" start: ") + oss.str() +
                                    "\n";
            start_str += "M624 " +
                         _encode_label_ids_to_base64(std::vector<size_t>(layer_object_label_ids.begin(), layer_object_label_ids.end())) +
                         "\n";
    ...
    if (!need_insert_timelapse_gcode_for_traditional &&
        is_BBL_Printer()) { // Equivalent to the timelapse gcode placed in layer_change_gcode
    ...
    auto insert_wrapping_detection_gcode = [this, &print, &print_z, &most_used_extruder]() -> std::string {
        ...
        if (print.config().enable_wrapping_detection && !print.config().wrapping_detection_gcode.value.empty()) {
            ...
            config.set_key_value("most_used_physical_extruder_id",
                                 new ConfigOptionInt(m_config.physical_extruder_map.get_at(most_used_extruder)));
            config.set_key_value("curr_physical_extruder_id",
                                 new ConfigOptionInt(m_config.physical_extruder_map.get_at(m_writer.filament()->extruder_id())));
            wrapping_gcode = this->placeholder_parser_process("wrapping_detection_gcode", print.config().wrapping_detection_gcode.value,
                                                              m_writer.filament()->id(), &config) +
                             "\n";
        }
        m_writer.set_current_position_clear(false);
    ...
    for (unsigned int extruder_id : layer_tools.extruders) {
        ...
        if (print.config().print_sequence == PrintSequence::ByLayer && m_enable_exclude_object &&
            ...
            for (InstanceToPrint& instance : filament_to_print_instances[extruder_id])
                filament_instances_id.emplace_back(instance.label_object_id);
        ...
        if (has_wipe_tower) {
            ...
            if (need_insert_timelapse_gcode_for_traditional && !has_insert_timelapse_gcode && m_writer.need_toolchange(extruder_id) &&
    ...
    if (need_insert_timelapse_gcode_for_traditional && !has_insert_timelapse_gcode) {
        ...
        if (m_config.nozzle_diameter.values.size() == 2 && m_support_traditional_timelapse &&
            m_config.timelapse_type.value == TimelapseType::tlTraditional &&
        ...
        if (filament_count_tmp * filament_count_tmp * heads_count_tmp == temp_flush_volumes_matrix.size()) {
            for (size_t idx = 0; idx < heads_count_tmp; ++idx) {
                ...
                std::transform(temp_flush_volumes_matrix.begin() + temp_begin_t, temp_flush_volumes_matrix.begin() + temp_end_t,
                               temp_flush_volumes_matrix.begin() + temp_begin_t, [temp_cfg_flush_multiplier_idx](double inputx) {
                                   return std::round(inputx * temp_cfg_flush_multiplier_idx);
                               });
            }
...
std::string GCode::extrude_path(ExtrusionPath path, std::string description, double speed)
{
    ...
    if (m_wipe.enable && FILAMENT_CONFIG(wipe)) {
        ...
        if (is_tree(this->config().support_type) &&
            (path.role() == erSupportMaterial || path.role() == erSupportMaterialInterface || path.role() == erSupportTransition)) {
            if ((m_wipe.path.first_point() - m_wipe.path.last_point()).cast<double>().norm() > scale_(0.2)) {
                ...
                for (; i < path.polyline.points.size(); i++) {
                    ...
                    if (dist < min_dist)
                        min_dist = dist;
                    if (min_dist < scale_(0.2) && dist > min_dist)
                        break;
                }
...
double GCode::calc_max_volumetric_speed(const double layer_height, const double line_width, const std::string co_str)
{
    ...
    if (cs.size() != 6 || std::all_of(cs.begin(), cs.end(), [](double v) { return v == 0; }))
...
std::string GCode::_extrude(const ExtrusionPath& path, std::string description, double speed)
{
    ...
    if (evaluate_adaptive_pa) {
        ...
        if (m_multi_flow_segment_path_average_mm3_per_mm > 0) {
            sprintf(buf, ";%sT%u MM3MM:%g ACCEL:%u BR:%d RC:%d OV:%d\n",
                    GCodeProcessor::reserved_tag(GCodeProcessor::ETags::PA_Change).c_str(), m_writer.filament()->id(),
            ...
            sprintf(buf, ";%sT%u MM3MM:%g ACCEL:%u BR:%d RC:%d OV:%d\n",
                    GCodeProcessor::reserved_tag(GCodeProcessor::ETags::PA_Change).c_str(), m_writer.filament()->id(), _mm3_per_mm,
    ...
    if (!variable_speed) {
        ...
        if ((std::abs(writer().get_current_speed() - F) > EPSILON) || (std::abs(_mm3_per_mm - m_last_mm3_mm) > EPSILON)) {
            ...
            if (_mm3_per_mm > 0 && FILAMENT_CONFIG(adaptive_pressure_advance) && FILAMENT_CONFIG(enable_pressure_advance) &&
                FILAMENT_CONFIG(adaptive_pressure_advance_overhangs) && !evaluate_adaptive_pa) {
...
LiftType GCode::to_lift_type(ZHopType z_hop_types)
{
    ...
    case ZHopType::zhtSlope: return LiftType::SlopeLift;
...
std::string GCode::retract(bool toolchange, bool is_last_retraction, LiftType lift_type, bool apply_instantly, ExtrusionRole role)
{
    ...
    bool needs_lift = toolchange || m_writer.filament()->retraction_length() > 0 || m_config.use_firmware_retraction;
...
std::string GCode::set_extruder(unsigned int new_filament_id, double print_z, bool by_object, int toolchange_temp_override)
{
    ...
    if (!m_writer.multiple_extruders) {
        ...
        if (!filament_start_gcode.empty()) {
            ...
            config.set_key_value("long_retraction_when_cut",
                                 new ConfigOptionBool(m_config.long_retractions_when_cut.get_at(new_filament_id)));
    ...
    if (m_writer.filament() != nullptr || m_start_gcode_filament != -1) {
        std::vector<float> flush_matrix(cast<float>(
            get_flush_volumes_matrix(m_config.flush_volumes_matrix.values, new_extruder_id, m_config.nozzle_diameter.values.size())));
        const unsigned int number_of_extruders =
            (unsigned int) (m_config.filament_colour.values.size()); // if is multi_extruder only use the fist extruder matrix
        if (m_writer.filament() != nullptr)
        ...
        // During the filament change, the extruder will extrude an extra length of grab_length for the corresponding detection, so the
        // purge can reduce this length.
        ...
        if (old_extruder_id != new_extruder_id) {
            ...
            int old_filament_id_in_new_extruder = m_writer.filament(new_extruder_id) != nullptr ? m_writer.filament(new_extruder_id)->id() :
                                                                                                  -1;
    ...
    // set volumetric speed of outer wall ,ignore per obejct,just use default setting
    float         outer_wall_volumetric_speed = get_outer_wall_volumetric_speed(m_config, *m_print, new_filament_id,
                                                                                get_extruder_id(new_filament_id));
    ...
    dyn_config.set_key_value("filament_tower_interface_purge_volume",
                             new ConfigOptionFloat(m_config.filament_tower_interface_purge_volume.get_at(new_filament_id)));
    {
