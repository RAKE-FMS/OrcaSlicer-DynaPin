...
[[nodiscard]] static const std::vector<Polygons> generate_overhangs(const TreeSupportSettings &settings, PrintObject &print_object, std::function<void()> throw_on_cancel)
{
    ...
    const int dynapin_debug_stage = DynaPin::effective_debug_stage(*print_object.print());
    auto dynapin_blockers = (dynapin_debug_stage >= 2) ? DynaPin::support_blockers_for_object(print_object) : std::vector<Polygons>{};
